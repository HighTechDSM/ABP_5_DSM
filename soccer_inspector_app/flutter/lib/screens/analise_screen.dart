...
Future<void> _baixarRelatorio() async {
  ...
  pdf.addPage(
    pw.MultiPage(
      build: (context) => [
        ...
        pw.Text(
            'Velocidade máxima: ${_jogador!['velocidadeMax']} km/h'),
        pw.Text(
            'Distância: ${_jogador!['distancia']} m'),
        pw.Text(
            'Sprints: ${_jogador!['sprints']}'),
  ...
  await Printing.sharePdf(
    bytes: bytes,
    filename:
        'relatorio_${_jogador!['numero']}.pdf',
  );
}
...
final mediaVelocidade = historico.isEmpty
    ...
    : historico
            .map((e) =>
                (e['velocidade'] as num?)?.toDouble() ?? 0)
            .reduce((a, b) => a + b) /
        historico.length;

final workload = historico.isNotEmpty
    ? ((historico.last['valor'] ?? 0) as num)
        .toDouble()
    : 0.0;

final velMaxNormalizado =
    (velocidadeMax.toDouble() / 40 * 100)
        .clamp(0.0, 100.0);

final velMediaNormalizada =
    (mediaVelocidade / 40 * 100)
        .clamp(0.0, 100.0);

final distanciaNormalizada =
    (distancia.toDouble() / 12000 * 100)
        .clamp(0.0, 100.0);

final sprintsNormalizado =
    (sprints.toDouble() / 30 * 100)
        .clamp(0.0, 100.0);

final workloadNormalizado =
    workload.clamp(0.0, 100.0);

final aceleracoesNormalizado =
    (((j['accelerations'] ?? 0) as num)
            .toDouble() /
        50 *
        100)
        .clamp(0.0, 100.0);
          ...
          child: SingleChildScrollView(
            ...
            child: SizedBox(
              ...
              child: Row(
                ...
                mainAxisAlignment: historico.length <= 8
                    ? MainAxisAlignment.spaceEvenly
                    : MainAxisAlignment.start,
                children: historico.isEmpty
                    ? []
                    : historico.map((hist) {
                        final valor = hist['valor'] is num
                            ? (hist['valor'] as num).toDouble()
                            : 0.0;

                        final porcentagem =
                            maxValor > 0 ? valor / maxValor : 0.0;

                        final safe =
                            porcentagem.clamp(0.05, 1.0);
          ...
          child: RadarChart(
            RadarChartData(
              ...
              tickBorderData: BorderSide(
                color: AppPalette.primary.withValues(alpha: .25),
              ),
              ...
              radarBorderData: BorderSide(
                color: AppPalette.primary.withValues(alpha: .30),
              ),
              ...
              dataSets: [
                RadarDataSet(
                  ...
                  dataEntries: [
                    ...
                    RadarEntry(value: desaceleracoesNormalizado),
                  ],
                ),
              ],
            ),
