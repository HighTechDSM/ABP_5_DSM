...
function Dashboard() {
  ...
  new Set(
    jogadores.flatMap((j) => j.historico.map((h) => h.data))
  )
).sort((a, b) => {
  // tenta ordenar por data (dd/mm)
  const [da, ma] = a.split("/").map(Number);
  const [db, mb] = b.split("/").map(Number);
  return da + ma * 100 - (db + mb * 100);
});

const radarData = todasDatas.map((data) => {
  const valoresDoDia = jogadores
    .map((j) => j.historico.find((h) => h.data === data)?.valor ?? 0)
    .filter((v) => v > 0);

  const total = valoresDoDia.reduce((s, v) => s + v, 0);
  const media = valoresDoDia.length ? total / valoresDoDia.length : 0;
